...
import { Pokemon, PokemonAbility, PokemonClient, PokemonEncounter, PokemonHabitat, PokemonType } from "pokenode-ts";
...
console.log(encounters)
    ...
    <div className="flex flex-row bg-slate-900 w-screen h-screen">
        <div id={"leftside"} className="flex flex-col items-center justify-center bg-slate-800 w-1/2">
        <h1 className="font-semibold text-2xl text-center text-white bg-slate-700 ">{`#${pokemon.id} ${pokemon.name.toLocaleUpperCase()}`}</h1>
        <Image className="object-cover" src={src} alt={pokemon.name + "-Image"} width={400} height={400} />
        </div>
        <div id={"rightside"} className="flex flex-col items-center justify-center bg-orange-50 w-1/2 px-4">
            <div className="flex flex-row gap-4 bg-orange-100 w-full">
                <h2 className="bg-orange-200">Typ: </h2>
                    <div className="bg-orange-300"> {pokemon.types.map((type:PokemonType) =>{
                    return (
            ...
            <h2 className="bg-orange-600">Größe: </h2>
                    <div className="bg-orange-700"> {pokemon.height} cm
                    </div>
                    </div>
                    <div className="flex flex-row gap-4 bg-orange-500">
            <h2 className="bg-orange-600">Gewicht: </h2>
                    <div className="bg-orange-700"> {pokemon.weight / 10} kg
                    </div>
                    </div>
            </div>
            ...
            <h2 className="bg-orange-600">Landschaft: </h2>
                    <div className="bg-orange-700"> {
                        encounters!.map((encounter)=>{
                            return( 
                                <div key={encounter.location_area.name} className="bg-orange-400">
